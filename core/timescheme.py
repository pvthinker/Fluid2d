...
class Timescheme(Param):
    ...
    def __init__(self,param,x):
        ...
        # internal arrays
        self.x   = zeros_like(x)
        self.dx0 = zeros_like(x)
        self.dx1 = zeros_like(x)

        if self.timestepping in ['RK3_SSP','AB3','RK3']:
            self.dx2 = zeros_like(x)

        if self.timestepping in ['LF','LFAM3']:
            self.xb = zeros_like(x)
    ...
    def set(self,rhs,timestepping):
        ...
        if self.timestepping in ['RK3_SSP']:
            self.kforcing=2

        if self.timestepping in ['Heun','LFAM3']:
            self.kforcing=1
    ...
    def AB2(self,x,t,dt):
        ...
        if self.first:
            ...
            x += ( (1.5+self.ab2_epsilon)*dt) * self.dx0 - ( (0.5+self.ab2_epsilon)*dt)*self.dx1
        self.dx1[:] = self.dx0
    ...
    def AB3(self,x,t,dt,**kwargs):
        ...
        if self.first:
            ...
            x += (23*dt/12.) * self.dx0 - (16*dt/12.)*self.dx1+(5*dt/12.)*self.dx2
        self.dx2[:] = self.dx1
        self.dx1[:] = self.dx0
