from numpy import ones,arange,meshgrid,sum,int8,sqrt,roll
...
class Grid(Param):
    def __init__(self,param):
        ...
        self.list_param=['nx','ny','npx','npy','Lx','Ly','myrank','nh','geometry','mpi','enforce_momentum','isisland']
        ...
        self.x1d = (arange(self.nxl)+0.5-self.nh + self.i0*self.nx//self.npx)*self.dx
        self.y1d = (arange(self.nyl)+0.5-self.nh + self.j0*self.ny//self.npy)*self.dy
    ...
    def set_msk(self):
        ...
        self.msk = ones((nyl,nxl),dtype=int8)

        if self.geometry=='perio':
            pass

        if self.geometry in ['xchannel','square','disc']:
            if self.j0 == npy-1: # northern boundary
                self.msk[-nh:,:] = 0
            if self.j0==0: # southern boundary
                self.msk[:nh,:] = 0
                
        if self.geometry in ['ychannel','square','disc']:
        ...
        if self.geometry == 'disc':
            r = sqrt( (self.xr/self.Lx-0.5)**2 + (self.yr/self.Ly-0.5)**2 )
            self.msk[r>=0.5]=0
    ...
    def finalize_msk(self):
        msk = self.msk
        self.msknoslip=self.msk-self.msknoslip
    ...
    def domain_integration(self,z2d):
        nh = self.nh
        integral = sum(z2d[nh:-nh,nh:-nh])*1.
...
if __name__ == "__main__":
    ...
    print(grid.yr[:,0])
    print(grid.msk)
    print('my coordinates in the subdomains matrix (%i,%i)'%(grid.j0,grid.i0))
