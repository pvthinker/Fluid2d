...
#        if( (n==n0)or(m==n0) ):
        ...
        if 'qgoperator' in param.keys():
            gridinfo.append({'n':n,'m':m,'np':np,'mp':mp,'np0':np0,'mp0':mp0,'omega':omega,
                             ...
                             'nh':nh,'ix':ix,'iy':iy,'dx':dx,'flag':flag,'lev':lev})
        else:
            gridinfo.append({'n':n,'m':m,'np':np,'mp':mp,'np0':np0,'mp0':mp0,'omega':omega,
                             'qgoperator':False,
                             'nh':nh,'ix':ix,'iy':iy,'dx':dx,'flag':flag,'lev':lev})
...
class Grid(object):
    import time
    def __init__(self,gridinfo):#flag,myrank,np,mp,incr,nh,n,m,comm,gather,msk0,A):
...
#            method = -1 # fill halo with 0s
        ...
        if self.flag=='peak':
            ...
            self.subdom=Subdomains(nh,self.nc,self.mc,comm,family,method=method)

        neighbours = set_neighbours(myrank,np0,mp0,ix,iy)
        self.halo=Halo(nh,nv,mv,comm,neighbours,method=method) # check method
    ...
    def set_timers_tozero(self):
        # time in sec 
        self.time={'smooth':0,'res':0,'sum':0,'norm':0,'interpolate':0,'restrict':0,'halo':0,'reduce':0,'split':0,'gather':0,'barrier':0}
        # number of calls
        self.ncalls={'smooth':0,'res':0,'sum':0,'norm':0,'interpolate':0,'restrict':0,'halo':0,'reduce':0,'split':0,'gather':0,'barrier':0}
...
#        a, b, c = -8, 1, 1
        ...
        for k in range(9):
            ...
            Acoef[J,I]=stencil[j+1,i+1]*coef*self.msk[J+j,I+i]*self.msk[J,I]
...
#    np = nbofproc/mp
    ...
    param={'np':np,'mp':mp,'nh':nh,'n':n,'m':m,'omega':0.85,
           'dx':dx,'n1':32,'n0':8,'method':'deep','nagglo':2}
