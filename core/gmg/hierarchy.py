    ...
    def Vcycle(self,lev1):
        ...
        for lev in range(lev1,deepest_lev):            
            ...
            if self.grid[lev].flag!='peak':
                ...
                finetocoarse(self.grid[lev],self.grid[lev+1],self.r[lev],self.b[lev+1])
            else:
                finetocoarse(self.grid[lev],self.grid[lev+1],self.b[lev],self.b[lev+1])
...
#---------------------------------------- 
    def print_timers(self):
        ...
        for lev in range(self.nlevs):
            ...
            comp=tt['smooth']+tt['res']+tt['sum']+tt['norm']+tt['interpolate']+tt['restrict']
            comp=tt['smooth']
            ...
            c3+=halo            
            c4+=barr
            print('%2i / comp =%5.3f / comm =%5.3f / halo =%5.3f / barrier =%5.3f / total=%5.3f'%(lev,comp,comm,halo,barr,comp+comm))

        print('----------------------------------------------------------')
        print('total      %5.3f /       %5.3f /       %5.3f /       %5.3f /       %5.3f'%(c1,c2,c3,c4,c1+c2))
        print('----------------------------------------------------------')
        for lev in range(self.nlevs):
            cc=self.grid[lev].ncalls
            print('%2i / nsmooth = %4i / nres = %4i/ nhalos = %4i'%(lev,cc['smooth'],cc['res'],cc['halo']))
        print('----------------------------------------------------------')
    ...
    def benchmark_comm(self):
        ...
        for lev in range(self.nlevs): 
            if self.myrank==0:
                print('%2i / halo = %5.3e / %5.3e'%(lev,comm1[lev]/nite,comm2[lev]/nite))
...
#    nite,res=gmg.Vsolve(x,b,{'maxite':20,'tol':1e-8})
    ...
    if myrank==0:
        walltime=(t1-t0)
        rescaledtime=-(walltime/nite)*(g.np*g.mp)/(gmg.nglo*gmg.mglo)/log(res)
        print('walltime (per ite)= %8.3e / rescaled time = %8.3e / average conv rate (per ite)= %4.2f digits / on %i x %i using %i x %i procs'%(walltime/nite,rescaledtime,-log(res)/nite/log(10),gmg.nglo,gmg.nglo,g.np,g.mp))
